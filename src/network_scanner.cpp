...
// ── Output routing ──────────────────────────────────────────────
//
// When a LogFn callback is provided, format the message and route it
// through the callback.  Otherwise fall back to portable::println
// (stdout/stderr).

template <typename... Args>
void log_out(const LogFn& log, std::format_string<Args...> fmt, Args&&... args)
{
    auto msg = std::format(fmt, std::forward<Args>(args)...);
    if (log) log(std::move(msg));
    else     portable::println("{}", msg);
}

template <typename... Args>
void log_err(const LogFn& log, std::format_string<Args...> fmt, Args&&... args)
{
    auto msg = std::format(fmt, std::forward<Args>(args)...);
    if (log) log(std::move(msg));
    else     portable::println(stderr, "{}", msg);
}
...
std::vector<uint32_t> discover_host_subnets(bool debug, const LogFn& log)
{
    ...
    for (const auto& name : candidates) {
        ...
        for (auto* rp = res; rp; rp = rp->ai_next) {
            ...
            if (!dup) {
                if (debug) {
                    uint32_t net = subnet;
                    log_out(log, "WSL2: resolved '{}' -> {}.{}.{}.{}, will scan {}.{}.{}.0/24",
                                      name,
                                           ...
                                           const LogFn& log)
{
    ...
    if (!wsa_init.ok) {
        log_err(log, "Failed to initialize Winsock");
        return devices;
    }
#endif
    ...
    if (sock == INVALID_SOCK) {
        log_err(log, "Failed to create UDP socket: error {}", get_last_socket_error());
        return devices;
    }
    ...
    if (::setsockopt(sock, SOL_SOCKET, SO_BROADCAST,
        ...
        log_err(log, "Failed to enable broadcast: error {}", get_last_socket_error());
        close_socket(sock);
        return devices;
    }
    ...
    if (::bind(sock, reinterpret_cast<sockaddr*>(&bind_addr), sizeof(bind_addr)) < 0) {
        log_err(log, "Failed to bind UDP socket: error {}", get_last_socket_error());
        close_socket(sock);
        return devices;
    }
    ...
    // 1. Limited broadcast (255.255.255.255) — works on the local L2 segment
    {
        ...
        if (debug) {
            if (sent > 0)
                log_out(log, "Sent VirCom search ({} bytes) -> 255.255.255.255:{}", sent, VIRCOM_PORT);
            else
                log_out(log, "Failed to send limited broadcast: error {}", get_last_socket_error());
        }
    }
    ...
    // 2. Per-interface directed broadcasts (e.g. 192.168.178.255)
    {
        ...
        for (const auto& baddr : bcast_addrs) {
            ...
            if (debug) {
                if (sent > 0)
                    log_out(log, "Sent VirCom search ({} bytes) -> {}:{}", sent, ip_str, VIRCOM_PORT);
                else
                    log_out(log, "Failed to send to {}: error {}", ip_str, get_last_socket_error());
            }
        }
    }
    ...
    if (!target_ip.empty()) {
        ...
        if (inet_pton(AF_INET, target_ip.c_str(), &dest.sin_addr) == 1) {
            ...
            if (debug) {
                if (sent > 0)
                    log_out(log, "Sent VirCom search ({} bytes) -> {}:{} (unicast)", sent, target_ip, VIRCOM_PORT);
                else
                    log_out(log, "Failed to send to {}: error {}", target_ip, get_last_socket_error());
            }
        } else if (debug) {
            log_out(log, "Invalid target IP for unicast scan: {}", target_ip);
        }
    }
    ...
    //    unicast probes regardless of platform.
    {
        ...
        for (const auto& cidr : extra_subnets) {
            ...
            if (inet_pton(AF_INET, cidr.c_str(), &addr) == 1) {
                ...
                log_out(log, "Invalid extra subnet: {}", cidr);
            }
        }
        for (uint32_t subnet : extra_nets) {
            ...
            if (debug) {
                log_out(log, "Sent 254 unicast probes to {}.{}.{}.1-254:{} (extra subnet)",
    ...
    if (is_wsl2()) {
        auto host_subnets = discover_host_subnets(debug, log);
        if (!host_subnets.empty()) {
            if (debug) {
                log_out(log, "WSL2 detected — sweeping {} discovered subnet(s) with unicast probes",
                                  host_subnets.size());
            }
            for (uint32_t subnet : host_subnets) {
                ...
                if (debug) {
                    log_out(log, "Sent 254 unicast probes to {}.{}.{}.1-254:{}",
                                      (subnet >> 24) & 0xFF, (subnet >> 16) & 0xFF,
                                      (subnet >> 8) & 0xFF, VIRCOM_PORT);
                }
            }
        } else if (debug) {
            log_out(log, "WSL2 detected but no additional subnets discovered via hostname resolution");
        }
    }

    if (debug) {
        log_out(log, "Waiting {} ms for responses ...", timeout_ms);
    }
    ...
    while (true) {
        ...
        if (n < 0) {
            ...
            #endif
            if (debug) {
                log_out(log, "recvfrom error: {}", err);
            }
            break;
        }
        ...
        if (n < static_cast<decltype(n)>(VIRCOM_PACKET_SIZE)) {
            if (debug) {
                log_out(log, "Ignoring short packet ({} bytes) from {}",
        ...
        if (parse_response(recv_buf.data(), static_cast<size_t>(n), dev)) {
            ...
            if (!duplicate) {
                if (debug) {
                    log_out(log, "Received response from {} (payload IP: {})",
                                      sender_ip, dev.ip_address);
                    log_out(log, "  Device name: {}", dev.device_name);
                    log_out(log, "  Module ID:   {}", dev.module_id);
                    log_out(log, "  MAC:         {}", dev.mac_address);
                    log_out(log, "  Subnet:      {}", dev.subnet_mask);
                    log_out(log, "  Gateway:     {}", dev.gateway);
                    log_out(log, "  DNS:         {}", dev.dns_server);
                    log_out(log, "  IP mode:     {} ({})", dev.ip_mode,
                                      dev.ip_mode == 1 ? "DHCP" : "Static");
                    log_out(log, "  Parameters:  {}", dev.parameters);
                }
                devices.push_back(std::move(dev));
            }
        } else if (debug) {
            log_out(log, "Failed to parse response from {}", inet_ntoa(sender_addr.sin_addr));
        }
    }
                        ...
                        const LogFn& log)
{
    ...
    socket_t sock = ::socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (sock == INVALID_SOCK) {
        log_err(log, "Failed to create UDP socket: error {}",
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
        for (const auto& dest : destinations) {
            ...
            if (sent > 0) {
                ok = true;
                if (debug) {
                    log_out(log, "Sent SET_CONFIG packet ({} bytes) -> {}:{} [attempt {}]",
                                      sent, ip_str, VIRCOM_PORT, attempt + 1);
                }
            } else if (debug) {
                log_out(log, "Failed to send SET_CONFIG to {}: error {}",
                   ...
                   const LogFn& log)
{
    ...
    if (!parse_ip_to_bytes(new_ip,      &packet[0x03]) ||
        ...
        log_err(log, "Invalid IP address format");
        return false;
    }
    ...
    if (debug) {
        log_out(log, "SET_CONFIG (Static IP) for device MAC {}:", device.mac_address);
        log_out(log, "  IP:      {}", new_ip);
        log_out(log, "  Mask:    {}", new_mask);
        log_out(log, "  Gateway: {}", new_gateway);
        log_out(log, "  DNS:     {}", new_dns);
    }

    return send_config_packet(packet, device.ip_address, debug, log);
}
...
std::optional<DiscoveredDevice> wait_for_device_reboot(
    ...
    const LogFn& log)
{
    log_out(log, "Waiting for device {} to reappear (timeout {}s) ...",
    ...
    while (true) {
        ...
        if (debug) {
            log_out(log, "Scanning for device {} ({:.0f}s / {:.0f}s) ...",
                              mac_address,
                              elapsed.count() / 1000.0,
                              wait_timeout_ms / 1000.0);
        }

        auto found = scan_network(SCAN_INTERVAL_MS, debug, {}, {}, log);

        for (const auto& d : found) {
            if (d.mac_address == mac_address) {
                log_out(log, "Device {} reappeared at {} ({})",
    ...
    log_err(log, "Timeout: device {} did not reappear within {}s.",
                      mac_address, wait_timeout_ms / 1000);
    return std::nullopt;
}
    ...
    const LogFn& log)
{
    ...
    if (debug) {
        log_out(log, "SET_CONFIG (DHCP) for device MAC {}:", device.mac_address);
    }

    if (!send_config_packet(packet, device.ip_address, debug, log)) {
        log_err(log, "Failed to send SET_CONFIG packet");
        return {};
    }

    auto result = wait_for_device_reboot(device.mac_address, wait_timeout_ms, debug, log);
    if (result) {
        return {*result};
    }
    return {};
                           ...
                           const LogFn& log)
{
    ...
    if (debug) {
        log_out(log, "SET_CONFIG (Modbus TCP) for device MAC {}:", device.mac_address);
        log_out(log, "  Transfer Protocol: Modbus TCP");
        log_out(log, "  Work Mode:         TCP Server");
        log_out(log, "  Port:              {}", port);
    }
                     ...
                     const LogFn& log)
{
    ...
    if (debug) {
        log_out(log, "SET_CONFIG (Port) for device MAC {}:", device.mac_address);
        log_out(log, "  Port: {}", port);
    }
                     ...
                     const LogFn& log)
{
    ...
    if (name.empty()) {
        log_err(log, "Device name must not be empty");
        return false;
    }
    if (name.size() > MAX_NAME_LEN) {
        log_err(log, "Device name '{}' is too long ({} chars, max {})",
    ...
    if (debug) {
        log_out(log, "SET_CONFIG (Name) for device MAC {}:", device.mac_address);
        log_out(log, "  New name: '{}'", name);
    }
